    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && data.user) {
          ...
          // 해금 상태이고 인적사항 정보가 일치할 때만 해금 상태 복원
          if (data.user.isUnlocked && data.user.sajuData && isProfileMatching) {
            setIsUnlocked(true);
            setCoachingData(data.user.sajuData.ai_coaching);
            sessionStorage.setItem("saju_report_unlocked", "true");
            sessionStorage.setItem("saju_report_data", JSON.stringify(data.user.sajuData));
          } else {
            // 정보가 다르거나 결제되지 않은 새로운 인적사항인 경우 즉각 잠금(Lock) 활성화
            setIsUnlocked(false);
        ...
        {activeTab === "report" && (
          <div className="flex flex-col gap-6">
            ...
            <div className="relative">
              ...
              >
                <h2 className="text-xl font-bold text-amber-400 pl-1">
