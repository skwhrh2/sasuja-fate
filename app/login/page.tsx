...
export default function LoginPage() {
  ...
  // 카카오 간편 가입/로그인 인가 코드 요청
  const handleKakaoLogin = () => {
    const clientId = process.env.NEXT_PUBLIC_KAKAO_CLIENT_ID || "test_kakao_client_id_12345";
    // 개발 단계(localhost)와 실서버(sasuja-fate.com) 도메인에 맞춰 redirectUri 자동 동적 계산
    const redirectUri = `${window.location.origin}/api/auth/kakao/callback`;
    
    // 추천인 정보를 쿠키에 30분 동안 임시 저장하여 OAuth 리다이렉트 콜백으로 전파
    if (referredBy) {
      ...
      <div className="max-w-md w-full flex flex-col gap-6">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl backdrop-blur-md">
          ...
          >
            ...
            {lang === "ko" ? "카카오로 3초 만에 시작하기" : lang === "en" ? "Start with Kakao" : lang === "ja" ? "Kakaoで始める" : lang === "zh" ? "用 Kakao 注册/登录" : "Bắt đầu bằng Kakao"}
          </button>
        </div>

        {/* 하단 링크 */}
        <div className="text-center">
          <Link href="/" className="text-xs text-slate-500 hover:text-amber-300 transition-colors">
            {t.backToHome}
          </Link>
        </div>
      </div>
